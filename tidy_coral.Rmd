...
1. make the column header a combo of rows 1 and 2
  - we want this to be `currentheader_row1`. So we want to combine them with a `_`
  - we want to identify row1 by name, not `buoy[1,]` because a) it's cryptic b) it will introduce silent problems if you run this code more than once
...
Nothing, which is great! But if we'd removed `buoy[1,]`, we'd be losing data (silently). 

OK. This is looking pretty good now. 

Let's say we want to compare these temperatures to the benthic species from above. Let's get a visual of what this data looks like, and then we'll think about how this relates with the benthic species. 
...
OK let's look at these dates — looks like we'll have to clean them up. Right now the dates are across several columns. Let's combine them into dates. 
...
We've got a lot of observations, our joined dataframe has way more rows than both the buoy data and the benthic data! What happened? How do we fix it?

```{r}
head(d_join[, c("id_name", "x", "y", "date", "hh_hr","mm_mn","WTMP_degC")])
```
